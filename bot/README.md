# 🤖 Midas Telegram Bot

Telegram бот для учёта финансов через Midas API.

## 🚀 Возможности

- ✅ Регистрация и авторизация
- 📝 Добавление транзакций через текст
- 🎤 Распознавание голосовых сообщений
- 📸 Распознавание чеков (OCR)
- 🤖 AI подсказки по категориям
- ✔️ Подтверждение перед сохранением
- 💰 Просмотр баланса

## 📋 Установка

### 1. Установи зависимости

```bash
cd bot
pip install -r requirements.txt
```

### 2. Создай бота в Telegram

1. Найди @BotFather в Telegram
2. Отправь `/newbot`
3. Следуй инструкциям
4. Скопируй токен

### 3. Настрой переменные окружения

Добавь в корневой `.env`:

```bash
# Telegram Bot
TELEGRAM_BOT_TOKEN=your_bot_token_from_botfather

# API URL (где запущен Midas API)
API_BASE_URL=http://localhost:8001

# OpenAI (уже есть)
OPENAI_API_KEY=sk-proj-...
```

### 4. Запусти бота

```bash
# Из корня проекта
python -m bot.main
```

Или:

```bash
cd bot
python main.py
```

## 📱 Использование

### Команды:

- `/start` - Начать работу
- `/register username email password` - Регистрация
- `/login username password` - Вход
- `/balance` - Показать баланс
- `/help` - Справка

### Примеры:

**Регистрация:**
```
/register ivan ivan@mail.com pass123
```

**Вход:**
```
/login ivan pass123
```

**Добавление расхода текстом:**
```
купил кофе за 25000 сум
```

**Голосовое:**
```
🎤 Запись: "потратил на такси тридцать тысяч"
```

**Фото чека:**
```
📸 Отправь фото чека
```

## 🔄 Логика работы

```
Пользователь → Текст/Голос/Фото
    ↓
AI парсит (через API /ai/parse-transaction)
    ↓
Бот: "Я понял так: Расход 25000 сум, категория: Транспорт. Верно?"
    ↓
Пользователь → ✅ Да / ❌ Нет
    ↓
Да → Сохраняет в БД (POST /transactions)
Нет → Просит повторить
```

## 📂 Структура

```
bot/
├── __init__.py
├── main.py          # Точка входа
├── config.py        # Конфигурация
├── api_client.py    # Клиент для Midas API
├── user_storage.py  # Хранение токенов и pending транзакций
├── handlers.py      # Обработчики команд и сообщений
├── requirements.txt # Зависимости
└── data/            # Локальные данные (создаётся автоматически)
    ├── users.json   # Токены пользователей
    └── pending.json # Ожидающие подтверждения транзакции
```

## 🔐 Безопасность

- Токены пользователей хранятся в `bot/data/users.json`
- Пароли НЕ хранятся (используются только для login)
- JWT токены действуют 30 дней

## 🐛 Дебаг

Логи выводятся в консоль:

```
2025-12-14 19:00:00 - __main__ - INFO - 🤖 Starting Midas Telegram Bot...
2025-12-14 19:00:05 - httpx - INFO - HTTP Request: POST /auth/login
```

## 🚀 Production

### Запуск в фоне (systemd):

```bash
sudo nano /etc/systemd/system/midas-bot.service
```

```ini
[Unit]
Description=Midas Telegram Bot
After=network.target

[Service]
Type=simple
User=your_user
WorkingDirectory=/path/to/midas
ExecStart=/usr/bin/python3 -m bot.main
Restart=always

[Install]
WantedBy=multi-user.target
```

```bash
sudo systemctl daemon-reload
sudo systemctl start midas-bot
sudo systemctl enable midas-bot
```

## ❓ FAQ

**Q: Бот не отвечает**
A: Проверь токен в `.env` и что API запущен

**Q: Ошибка авторизации**
A: Убедись что API доступен по `API_BASE_URL`

**Q: AI не распознаёт**
A: Проверь что `OPENAI_API_KEY` настроен в API

---

**Made with ❤️ for easy expense tracking**
